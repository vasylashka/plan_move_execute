...
class BaseLocalPlanner(Node):
    ...
    def get_environment_data(self):
        ...
        # Using a simple wait for demo; in production, use a more robust async pattern
    ...
    def control_loop(self):
        """The main execution loop that remains the same regardless of strategy"""
        time.sleep(2.0)
        while rclpy.ok() and self.planning_active:
            ...
            # Check if reached
            if np.linalg.norm(self.current_joints - self.q_goal) < 0.01:
            ...
            # Get environmental data (distances/jacobians)
